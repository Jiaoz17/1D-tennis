...
// Debug
let debugText = "";
let textColor = [61, 145, 35]; 

function preload() {
  // Load sound files
  ballHitSound = loadSound('sound/ball-1.wav');
  ohhSound = loadSound('sound/cro-1.wav');
  winSound = loadSound('sound/apl-1.wav');
}
...
function setup() {
  ...
  ballHitSound.setVolume(0.5);  // 50% volume
  ohhFieldSound.setVolume(0.7);  // 70% volume
  winSound.setVolume(0.8);  // 80% volume
  
   // Set a flag when sounds are loaded
  soundsLoaded = true;
}
...
function drawUShapedCrowd(cellSize, topY, bottomY, screenWidth, screenHeight) {
  // Use the same size as court squares
  spectatorSize = cellSize;
  let spacing = spectatorSize;
  
  // Calculate the number of spectators that can fit horizontally
  let spectatorsPerRow = Math.floor(screenWidth / spectatorSize);
  
  // Calculate if ball is in left or right half
  let inLeftHalf = ball < gridSize / 2;
  
  // Fixed colors - no brightness variation
  let redColor = [181, 42, 42];   // Original red color for all red crowd
  let blueColor = [42, 107, 181]; // Original blue color for all blue crowd
  
  // Create a flickering effect based on frame count
  // This will cause spectators to appear and disappear
  let shouldFlickerThisFrame = frameCount % 50 < 45; // Toggle every 5 frames
  
  // Draw top row (horizontal part of U) - all red
  for (let i = 0; i < spectatorsPerRow; i++) {
    let x = i * spectatorSize;
    
    // Determine if this column is in group 1 (odd) or group 2 (even)
    let isOddColumn = i % 2 === 0; // 0-indexed, so even indices are odd columns
    
    // Only draw if this group should be visible
    // When ball is in left court, odd columns flicker (appear/disappear)
    // When ball is in right court, even columns flicker (appear/disappear)
    let shouldFlicker = (isOddColumn && inLeftHalf) || (!isOddColumn && !inLeftHalf);
    
    // If in flickering group, check frame count to determine visibility
    // If not in flickering group, always draw
    if (!shouldFlicker || (shouldFlicker && shouldFlickerThisFrame)) {
      // All red spectators have the same color
      fill(redColor[0], redColor[1], redColor[2]);
      rect(x, topY, spectatorSize - 2, spectatorSize - 2);
    }
  }
  
  // Draw bottom row (horizontal part of U) - all blue
  for (let i = 0; i < spectatorsPerRow; i++) {
    let x = i * spectatorSize;
    
    // Same grouping logic as top row
    let isOddColumn = i % 2 === 0;
    let shouldFlicker = (isOddColumn && inLeftHalf) || (!isOddColumn && !inLeftHalf);
    
    // Only draw if this spectator should be visible this frame
    if (!shouldFlicker || (shouldFlicker && shouldFlickerThisFrame)) {
      // All blue spectators have the same color
      fill(blueColor[0], blueColor[1], blueColor[2]);
      rect(x, bottomY, spectatorSize - 2, spectatorSize - 2);
    }
  }
  
  // Calculate vertical height available for side crowds
  let verticalSpace = bottomY - (topY + spectatorSize);
  let sideRows = Math.floor(verticalSpace / spacing);
  ...
  for (let i = 0; i < sideRows; i++) {
    ...
    // Determine if in top or bottom half to match color with horizontal rows
    let isTopHalf = yPos < screenHeight / 2;
    
    // Determine if this row should flicker
    let isOddRow = i % 2 === 0;
    let shouldFlicker = (isOddRow && inLeftHalf) || (!isOddRow && !inLeftHalf);
    
    // Only draw if this spectator should be visible this frame
    if (!shouldFlicker || (shouldFlicker && shouldFlickerThisFrame)) {
      if (isTopHalf) {
        // Red color for top half
        fill(redColor[0], redColor[1], redColor[2]);
      } else {
        // Blue color for bottom half
        fill(blueColor[0], blueColor[1], blueColor[2]);
      }
      
      // Draw left side spectator
      rect(0, yPos, spectatorSize - 2, spectatorSize - 2);
    }
  }
  
  // Right vertical part of the U - mirror of left side
  for (let i = 0; i < sideRows; i++) {
    let yPos = topY + spectatorSize + i * spacing;
    
    // Match color scheme with left side
    let isTopHalf = yPos < screenHeight / 2;
    
    // Flicker logic for right side
    let isOddRow = i % 2 === 0;
    let shouldFlicker = (isOddRow && inLeftHalf) || (!isOddRow && !inLeftHalf);
    ...
    if (!shouldFlicker || (shouldFlicker && shouldFlickerThisFrame)) {
      ...
      // Draw right side spectator at right edge of screen
      rect(screenWidth - spectatorSize, yPos, spectatorSize - 2, spectatorSize - 2);
    }
  }
}
...
function handleBounce() {
  if (bounceActive) {
    ...
    if (ball === 3 && (key === 'w' || key === 'W') && firstRally) {
      ...
      if (soundsLoaded) ballHitSound.play();
    }
...
function moveBall() {
  ...
  if (ball < 0 || ball >= gridSize) {
    if (!ballOutsideField) {
      ...
      if (soundsLoaded) ohhFieldSound.play();
