...
// New force control for left player
let leftPlayerForce = 0;      // Store the force value for left player
let lastInputValue = "";      // Store the input buffer
...
function draw() {
  ...
  fill(255);
  textSize(12);
  text("Game controls: Left player (number + W, A, D) - Right player (I, J, L)", 10, 15);
  
  // Display current force value for left player
  fill(255);
  textSize(16);
  text(`Current left player force: ${leftPlayerForce}`, width / 2 - 100, height - 50);
...
function performBounce(player) {
  ...
  console.log("Performing bounce for", player, "with force:", leftPlayerForce);
  
  if (player === "left") {
    // Use the force value for left player
    if (leftPlayerForce > 0) {
      console.log("Using force value:", leftPlayerForce);
      
      // Map force to appropriate speed and decay rate - keep within reasonable bounds
      ballSpeed = floor(map(leftPlayerForce, 1, 10, 8, 2));
      ballSpeed = constrain(ballSpeed, 2, 8);
      
      speedDecayRate = map(leftPlayerForce, 1, 10, 0.5, 0.2);
    ...
    // Right player still uses tap count
    if (finalTapCount > 0) {
  ...
  tapCount = 0;
  finalTapCount = 0;
  // We do NOT reset leftPlayerForce here so it can be reused
  firstRally = false;
}
...
function keyPressed() {
  ...
  // Direct handling of number keys for force
  if (key >= '0' && key <= '9') {
    // Set force value immediately
    leftPlayerForce = (key === '0') ? 10 : parseInt(key);
    debugText = `Left player force set to: ${leftPlayerForce}`;
    
    // Also update input buffer
    lastInputValue += key;
    console.log("Input buffer updated:", lastInputValue);
  }
  ...
  if (pressedKey === 'w') {
    wPressed = true;
    // W key now just sets the flag - force is already set by number keys
    console.log("W pressed, current force:", leftPlayerForce);
  }
...
// Add keyTyped function for more reliable text input
function keyTyped() {
  ...
  // Only handle number keys here as backup for keyPressed
  if (key >= '0' && key <= '9') {
    // Set force value immediately
    leftPlayerForce = (key === '0') ? 10 : parseInt(key);
    debugText = `Left player force set to: ${leftPlayerForce}`;
...
function updateDebugText() {
  ...
  // Show different info based on which player's turn it is
...
function resetRally() {
  ...
  tapCount = 0;
  finalTapCount = 0;
  leftPlayerForce = 0;  // Reset force value
  lastInputValue = "";  // Clear input buffer
